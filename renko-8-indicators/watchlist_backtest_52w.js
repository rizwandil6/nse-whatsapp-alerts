'use strict';
/** DarvasBox (confirmed-box version), 23-stock watchlist, trailing 52 weeks
 * (2025-07-23 to 2026-07-21) via watchlist_52w_cache.json. Same engine as
 * watchlist_backtest.js, pointed at the 52-week cache instead of the
 * 60-trading-day one. */
...
const cache = JSON.parse(fs.readFileSync(path.join(__dirname, 'watchlist_52w_cache.json'), 'utf8'));
function getFiveMin(symbol) { return cache[symbol] ? cache[symbol].fiveMin : null; }
...
fs.writeFileSync(path.join(__dirname, `trades_watchlist_52w_DarvasBox_${brickLabel}pct.json`), JSON.stringify(allTrades, null, 1));

console.log(`\n=== DarvasBox (confirmed box) @ ${BRICK_PCT * 100}% brick, 52-week, ${WATCHLIST.length}-stock watchlist ===\n`);
