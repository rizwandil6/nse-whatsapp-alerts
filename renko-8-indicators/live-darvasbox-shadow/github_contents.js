 ...
 * renko-python-backtest/live/github_contents.js (a fresh, independent copy
 * -- kept self-contained within this directory, not a cross-directory
 * require, so it doesn't run into Railway's Root Directory build-scoping
 * issue).
 *
...
function authHeaders(token) {
  return {
    ...
    'User-Agent': 'darvasbox-shadow-0.25pct-1pctSL',
  };
}
...
async function getRemoteFile(token, repoRelPath, branch) {
  ...
  if (meta.encoding === 'base64' && meta.content) {
    ...
    const rawRes = await fetch(rawUrl, { headers: { 'User-Agent': 'darvasbox-shadow-0.25pct-1pctSL' } });
