'use strict';
/** DarvasBox (confirmed-box version), 23-stock watchlist, trailing 3 years
 * (2023-07-24 to 2026-07-21) via watchlist_3y_cache.json. Same engine as
 * watchlist_backtest.js, pointed at the 3-year cache instead of the
 * 60-trading-day one. */
...
fs.writeFileSync(path.join(__dirname, `trades_watchlist_3y_DarvasBox_${brickLabel}pct.json`), JSON.stringify(allTrades, null, 1));

console.log(`\n=== DarvasBox (confirmed box) @ ${BRICK_PCT * 100}% brick, 3-year, ${WATCHLIST.length}-stock watchlist ===\n`);
