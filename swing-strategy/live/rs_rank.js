...
/**
 * A fresh, independent copy of rs-momentum-strategy/live/rs_rank.js -- kept
 * self-contained within this directory, not a cross-directory require, so it
 * doesn't run into Railway's Root Directory build-scoping issue (same pattern
 * as github_contents.js elsewhere in this repo). Used by trimmed_runner.js to
 * compute rsRankAtEntry on the SAME day a signal fires, rather than reading
 * rs-momentum-strategy's own published snapshot (which lags a day given the
 * two services' run-order -- see trimmed_runner.js's comment). If the ranking
 * methodology here ever changes, update both copies.
 *
