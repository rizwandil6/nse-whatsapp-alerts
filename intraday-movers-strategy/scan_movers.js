...
function main() {
  ...
  for (let d = 1; d < allDates.length; d++) {
    ...
    for (const { candidate, direction } of [
      { candidate: topGainer, direction: 'LONG' },
      { candidate: topLoser, direction: 'SHORT' },
    ]) {
      ...
      const gapsFurther = direction === 'LONG' ? todayCandle.open > candidate.prevClose : todayCandle.open < candidate.prevClose;
